...
def _error_cases(
    records: dict[str, list[dict[str, str]]], probabilities: torch.Tensor, limit: int = 20
    ...
    false_positive_indices = [
        ...
        if int(row["label"]) == 0 and probabilities[index] >= 0.5
    ]
    false_negative_indices = [
        index for index, row in enumerate(val_rows)
        if int(row["label"]) == 1 and probabilities[index] < 0.5
    ]
    ...
    if args.checkpoint:
        ...
        for grouping, values in groups.items():
            ...
            for value in sorted(set(values)):
                ...
                metrics = classification_metrics(val_y[mask], probabilities[mask])
        ...
        report["representative_errors"] = _error_cases(records, probabilities)
