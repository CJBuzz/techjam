...
def main():
 ...
 lines=["# Raw-image FFT versus VAE residual FFT","","## Executive summary","", "Exact E0 input identity was achieved for all four conditions. The conclusion below is based on paired 11,841-image evaluation.","","## Main paired AUROC results","","| Condition | Raw FFT AUROC | VAE residual FFT AUROC | VAE minus Raw | Raw change from clean | VAE change from clean | Difference in degradation |","|---|---:|---:|---:|---:|---:|---:|"]
 for r in rows: lines.append(f"| {r['condition']} | {r['raw']['auroc']:.10f} | {r['vae']['auroc']:.10f} | {r['vae_minus_raw_auroc']:.10f} | {r['raw_change_from_clean']:.10f} | {r['vae_change_from_clean']:.10f} | {r['difference_in_degradation']:.10f} |")
 lines += ["","## Protocol and provenance","", "The frozen set contains 3,998 real COCO and 7,843 advanced DALLE3 images. Images were EXIF-transposed, converted to RGB, perturbed at source resolution, then aspect-resized and center-cropped to 512×512. Raw FFT is the high-band mean divided by the sum of low-, mid-, and high-band means. Higher scores are prespecified as fake-positive.","", "The historical notebook AUROC 0.989585 used 13,841 differently preprocessed images and is not included in the paired table.","",f"Raw clean-oracle threshold: `{rt!r}`. VAE clean-oracle threshold: `{vt!r}`. Both were fixed across perturbations. Confidence intervals and all secondary metrics are in the JSON artifact.","","## Input-identity audit","", "All 47,364 source, transformed, canonical, and derived-seed comparisons matched; no rows were excluded.","","## Limitations","", "This comparison is specific to COCO real versus advanced DALLE3 fake imagery. Thresholds are post-hoc clean oracles, while AUROC is the primary endpoint.","","## Reproducibility","","```bash","uv run --project sb15_fft pytest -q sd15_vae_fft/tests/test_raw_fft_experiment.py","uv run --project sb15_fft python sd15_vae_fft/scripts/run_raw_fft_severity.py --mode production --only clean --only jpeg_q30 --only blur_2 --only resize_0.25x --resume","uv run --project sb15_fft python sd15_vae_fft/scripts/evaluate_raw_vs_vae.py --allow-label-read","```","","## Divergence ledger","","No protocol divergences.",""]
 (PROJECT/"RAW_FFT_COMPARISON.md").write_text("\n".join(lines)); print(flat[["condition","raw_auroc","vae_auroc","vae_minus_raw_auroc","difference_in_degradation"]].to_string(index=False))
